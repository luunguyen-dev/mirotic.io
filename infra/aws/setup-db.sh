...
DB_NAME="${DB_NAME:-mirotic}"
DB_USER="${DB_USER:-mirotic}"
...
aws ec2 authorize-security-group-ingress --region "$AWS_REGION" --group-id "$SG_ID" \
  ...
  --tag-specifications 'ResourceType=security-group-rule,Tags=[{Key=app,Value=mirotic}]' \
...
sudo docker volume create mirotic_pg >/dev/null 2>&1 || true
if sudo docker ps -a --format '{{.Names}}' | grep -qx 'mirotic-pg'; then
  echo "   container mirotic-pg đã tồn tại"
else
  sudo docker run -d --name mirotic-pg --restart unless-stopped \
    -e POSTGRES_DB="$DB_NAME" -e POSTGRES_USER="$DB_USER" -e POSTGRES_PASSWORD="$DB_PASS" \
    -p 5432:5432 -v mirotic_pg:/var/lib/postgresql/data postgres:16 >/dev/null
  echo "   ✓ tạo container mirotic-pg (postgres:16)"
fi
REMOTE
...
if command -v psql >/dev/null 2>&1; then
  ...
  ssh -i "$SSH_KEY" "$SSH_USER@$HOST" "sudo docker exec -i mirotic-pg psql -U '$DB_USER' -d '$DB_NAME'" < schema.sql
fi
echo "   ✓ schema xong"
